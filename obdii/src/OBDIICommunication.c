...
static int setupDaemonCommunication() {
	...
	// Even though this is a connection-less socket, by using connect we can use
	// send and recv calls instead of sendto/recvfrom
...
OBDIICommandSet OBDIIGetSupportedCommands(OBDIISocket* socket) {
	...
	// response = OBDIIPerformQuery(socket,
	// OBDIICommands.mode1SupportedPIDs_61_to_80);
...
exit:
	numCommands = _BitsSet(supportedCommands._mode1SupportedPIDs._1_to_20) +
				  _BitsSet(supportedCommands._mode1SupportedPIDs._21_to_40) +
				  _BitsSet(supportedCommands._mode1SupportedPIDs._41_to_60) +
				  _BitsSet(supportedCommands._mode1SupportedPIDs._61_to_80) +
				  _BitsSet(supportedCommands._mode9SupportedPIDs);
...
OBDIIResponse OBDIIPerformQuery(OBDIISocket* socket, OBDIICommand* command) {
	...
	// Receive the response
	int responseLength = command->expectedResponseLength == VARIABLE_RESPONSE_LENGTH ? MAX_ISOTP_PAYLOAD
	...
	if (retval < 0 ||
		(command->expectedResponseLength != VARIABLE_RESPONSE_LENGTH && retval != command->expectedResponseLength)) {
		UnlockIfNecessary(socket);
		return response;
	}
