...
/**
 * Represents an OBDII response, obtained through a call to
 * `OBDIIDecodeResponseForRequest` or `OBDIIPerformQuery`.
 *
 * For most commands, the diagnostic data is accessible via one of
 * `numericValue`, `bitfieldValue`, or `stringValue`. A few commands
 * (`OBDIICommands.DTCs` and the `OBDIICommands.oxygenSensor*` commands) have
 * specific properties in the response object through which their data is
 * accessed. Each command documents the type of data it returns. However, it is
 * possible to dynamically determine the type, via the command's `responseType`
 * property. E.g.
 *
 ...
 *    }
 *
 * Make sure to call `OBDIIResponseFree` when you are done with a given response
...
/** Type for a function that can decode the raw response payload for a
 * particular command.
 *
 * \param response A pointer to the `OBDIIResponse` object which should be
 * filled with the decoded value \param responsePayload The raw response bytes
 * to be decoded \param responseLen The size of the `responsePayload` buffer
 */
 ...
 *
 * You don't create instances of this type yourself. Rather, the list of
 * commands is predefined, with each command represented by a property on the
 * `OBDIICommands` global variable. For example, `OBDIICommands.engineRPMs` is a
 * pointer to the unique command instance for requesting the engine RPMs. The
 * commands are also accessible via the `OBDIIMode1Commands` and
 * `OBDIIMode9Commands` global variables, which are indexed by the command's
 * PID. For example,
 *
...
typedef struct OBDIICommand {
	...
	char* name;
	/** The raw request payload, which for all commands is simply a (mode, PID)
	 * tuple. */
	unsigned char payload[2];
	/** The type of data contained in the response for this command. */
	OBDIIResponseType responseType;
	/** The expected length of a payload containing the raw response to this
	 * command. Equal to `VARIABLE_RESPONSE_LENGTH` for commands whose response
	 * payloads have variable length. */
	short expectedResponseLength;
	/** A pointer to a function that can decode a raw response payload for this
	 * command. */
 ...
 *
 * The OBDII standard specifies a large number of diagnostic commands, but a
 * particular vehicle will likely only support a subset. The
 * `OBDIIGetSupportedCommands` function can be used to query a vehicle for the
 * set of commands it supports. For example,
 *
 ...
 * OBDIICommandGetMode(command), OBDIICommandGetPID(command), command->name);
 *     }
...
/** Global variable used to obtain a reference to a particular command, e.g.
 * `OBDIICommands.vehicleSpeed` */
extern struct OBDIICommands OBDIICommands;
/** Global variable used to obtain a reference to a mode 1 command by PID, e.g.
 * `OBDIIMode1Commands[0x1F]` (same as OBDIICommands.runtimeSinceEngineStart) */
extern OBDIICommand OBDIIMode1Commands[79];
/** Global variable used to obtain a reference to a mode 9 command by PID, e.g.
 * `OBDIIMode9Commands[0x02]` (same as OBDIICommands.VIN) */
 ...
 * Make sure to call `OBDIIResponseFree` when you are done with the response.
 */
 ...
 * \param response A pointer to the response object whose resources should be
 * freed.
 */
 ...
 * \param command The command whose presence in `commandSet` should be
 * determined.
 *
 ...
 *
 * \param commandSet A pointer to the command set whose resources should be
 * freed.
 */
